...
{
	public enum SolverTypes { CGMLU, LOSLU, LOSLUParallel }
	...
	public class LOSLUParallel : ISolver
	{
		public int MaxIterCount { get; set; } = 1000;
		public int IterCount { get; set; } = 0;
		...
		public double[] Solve(IMatrix matrix, double[] B)
		{
			...
			Difference = Utilities.DotProductParallel(r, r);

			while (IterCount < MaxIterCount && Difference >= Eps && Utilities.ErrorParallel(x, xPrev) >= 1.0e-10 * 1.0e-10)
			{
				// Calculate alpha
				double dotP = Utilities.DotProductParallel(p, p);
				double a = Utilities.DotProductParallel(p, r) / dotP;
				...
				matrix.Multiply(Ax, temp);
				Forward(LU, Ax, temp);
				double b = -Utilities.DotProductParallel(p, Ax) / dotP;
