...
namespace FEM
{
	public enum AreaSide { Left, Top, Right, Bottom, TopRight, TopBottom }
	...
	public class GridBuilder2
	{
		...
		public GridBuilder2(AreaInfo info)
		{
			Info = info;

			Grid = new Mesh();

			int pointCount = Info.HorizontalNodeCount * (Info.FirstLayerVerticalNodeCount + Info.SecondLayerVerticalNodeCount - 1) - 1;
			Points = new List<Point>();
		...
		void BuildTriangles()
		{
			...
			for (int j = 0; j < Info.HorizontalNodeCount - 2; j++)
			{
				FiniteElement e1 = new FiniteElement();
				FiniteElement e2 = new FiniteElement();
			...
			for (int i = 1; i < Info.FirstLayerVerticalNodeCount - 1; i++)
			{
				for (int j = 0; j < Info.HorizontalNodeCount - 1; j++)
				{
					FiniteElement e1 = new FiniteElement();
					...
					int p1 = i * Info.HorizontalNodeCount + j - 1;
					int p2 = i * Info.HorizontalNodeCount + j;
					int p3 = (i + 1) * Info.HorizontalNodeCount + j - 1;
					int p4 = (i + 1) * Info.HorizontalNodeCount + j;
		...
		void BuildBoundary()
		{
			...
			if (Info.Conditions[AreaSide.Top].Item1 == ConditionType.First)
			{
				for (int i = 1; i < Info.HorizontalNodeCount - 1; i++)
				{
					Edge edge = new Edge();
					edge.V1 = i;
					edge.V4 = i + 1;
			...
			if (Info.Conditions[AreaSide.Bottom].Item1 == ConditionType.First)
			{
				...
				{
					Edge edge = new Edge();
					edge.V1 = Info.HorizontalNodeCount * (Info.FirstLayerVerticalNodeCount - 1) + i - 1;
					edge.V4 = Info.HorizontalNodeCount * (Info.FirstLayerVerticalNodeCount - 1) + i;
					edge.Function = Info.Conditions[AreaSide.Top].Item2;
					FB.Edges.Add(edge);
				}
			}
			...
			// TopRight
			if (Info.Conditions[AreaSide.TopRight].Item1 == ConditionType.First)
			{
				Edge edge = new Edge();
				edge.V1 = 0;
				edge.V4 = Info.HorizontalNodeCount;
				edge.Function = Info.Conditions[AreaSide.TopRight].Item2;
				FB.Edges.Add(edge);
			}

			// TopBottom
			if (Info.Conditions[AreaSide.TopBottom].Item1 == ConditionType.First)
			{
				Edge edge = new Edge();
				edge.V1 = Info.HorizontalNodeCount - 1;
				edge.V4 = Info.HorizontalNodeCount;
				edge.Function = Info.Conditions[AreaSide.TopBottom].Item2;
				FB.Edges.Add(edge);
			}
		}
	}
