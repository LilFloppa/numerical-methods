...
namespace SlaeSolver
{
	...
	public class LOSLU : ISolver
	{
		...
		public double[] Solve(IMatrix matrix, double[] B)
		{
			...
			Difference = Vector.Dot(r, r);

			while (IterCount < MaxIterCount && Difference >= Eps && Vector.Error(x, xPrev) >= 1.0e-10)
			{
				// Calculate alpha
				double dotP = Vector.Dot(p, p);
				double a = Vector.Dot(p, r) / dotP;
